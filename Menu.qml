...
Item {
  ...
  function rebuildDisplay() {
    ...
    var calculation = Calculator.row(query)
    if (calculation) displayModel.insert(0, calculation)
    layoutSerial += 1
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (row.kind === "calculator-error") {
      return
    } else if (row.kind === "calculator") {
      if (clipboardCopy.running) return
      clipboardCopy.result = row.label
      clipboardCopy.serial = root.requestSerial
      clipboardCopy.query = root.filterText
  ...
  Process {
    ...
    command: ["wl-copy", "--type", "text/plain", "--", result]
    onExited: function(exitCode, exitStatus) {
      if (serial !== root.requestSerial || query !== root.filterText) return
      if (exitCode === 0 && exitStatus === 0) {
        root.cancel()
      } else {
        // Keep the result visible when the clipboard is unavailable.
        if (displayModel.count && displayModel.get(0).kind === "calculator")
          displayModel.setProperty(0, "detail", "Could not copy · try again")
      }
    }
  }
